...
import services.util.JobServerNamedRdds

/**
 * Created by king on 15-10-10.
 */
class RddManagerActorSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
with FunSpecLike with Matchers with BeforeAndAfter with BeforeAndAfterAll {

  def this() = this(ActorSystem("RddManagerActorSpec"))
  ...
  before {
    namedRdds.getNames().foreach(namedRdds.destroy)
  }

  override def afterAll(): Unit = {
    system.stop(rddManager)
    sc.stop()
  ...
  describe("NamedRdds") {
    ...
    it("should include underlying exception when error occurs") {
      ...
      val err = intercept[RuntimeException] {
        namedRdds.getOrElseCreate("rdd", errorFunc)
      }
